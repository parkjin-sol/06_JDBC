...
public class JDBCExample1 {
	public static void main(String[] args) {
		...
		/*
		 * 입력 받은 아이디가 포함된 사용자의 사용자 번호, 아이디, 이름,가입일을
		...
		try {
			...
			/*
			 * 5. Statement 객체를 이용해서 SQL을 DB로 전달 후 수행 1) 문 : executeQuery() -> ResultSet으로
			 * 반환 2) DML문 : executeUpdate() -> 결과 행의 개수(int) 반환
			 */
			String sql = sb.toString();
			rs = stmt.executeQuery(sql);

			/*
			 * (5번 SQL이 SELECT인 경우에만) 6. 조회 결과가 저장된 ResultSet을 1행씩 접근하여 각 행에 기록된 컬럼 값 얻어오기
			 */
